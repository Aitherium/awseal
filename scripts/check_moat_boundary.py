...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awseal
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awseal-*.whl dist/*.tar.gz ...]
...
_KEYSTONES = ("awseal/__init__.py", "awseal/cli.py", "awseal/digest.py", "awseal/keys.py", "awseal/seal.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awseal-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awseal/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awseal/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awseal/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awseal/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awseal/y.py": "from awseal.client import call\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awseal-0.0.0.tar.gz"
